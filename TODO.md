...
3. **Suivez les etapes proposees a la racine du projet.** Chaque fichier contient des
   marqueurs `TODO (Sx-n)` qui correspondent aux etapes d'un enonce de TP. Vous
   completez ces TODO, et seulement ceux-la : le reste du code est fourni et
   fonctionnel.
4. **Vous pouvez aller plus loin** (sections "Pour aller plus loin" des TP, idees
   personnelles), mais **au minimum vous faites tous les exercices proposes**.

> Le seul fichier a modifier pour brancher votre dataset est `mlproject/config.py`
> (TP S0). `data.py` et `features.py` lisent automatiquement votre configuration :
> vous n'avez pas a les toucher.
...
Le package `mlproject` est rendu importable via `PYTHONPATH=.` (deja gere par le
Makefile). Adaptez `mlproject/config.py` a votre dataset (TP S0), generez les
donnees puis verifiez la baseline :

```bash
python -m mlproject.prepare_data            # genere data/dataset.csv
python -m mlproject.train                   # doit afficher f1=... roc_auc=...
```

## Contenu du projet (a la racine)

```
README.md                ce fichier de presentation court
TODO.md                  ce guide pedagogique
Makefile                 cibles d'installation fournies, les autres a completer
pyproject.toml           dependances du projet (uv)
mlproject/
  config.py              configuration a adapter a votre dataset   <- TP S0
  prepare_data.py        preparation du dataset (fourni, adapte)   <- TP S0
...
Toutes les commandes s'executent depuis la racine du projet (le Makefile fixe
`PYTHONPATH=.`, ex. `python -m mlproject.train`).
